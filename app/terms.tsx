...
export default function TermsScreen() {
  ...
  const TermsCard = ({ icon: Icon, title, content, color = "#6366f1", fullWidth = false }: any) => (
    <View className={`${fullWidth ? 'w-full' : 'w-[48%]'} bg-white/5 p-5 rounded-[32px] border border-white/5 mb-4`}>
      <View className="flex-row items-center space-x-3 mb-4">
        ...
        <Typography weight="black" className="text-[10px] text-white italic tracking-widest uppercase flex-1">{title}</Typography>
      </View>
      <Typography weight="bold" className="text-[8px] text-white/40 leading-4 uppercase tracking-wider italic">
        {content}
      </Typography>
    </View>
  );
    ...
    <SafeAreaView className="flex-1 bg-[#020408]">
      <ScrollView className="flex-1 p-6" showsVerticalScrollIndicator={false}>
        ...
        <View className="flex-row items-center justify-between mb-12">
           <View className="flex-row items-center space-x-3">
              ...
              <View>
                <Typography weight="black" className="text-[8px] text-indigo-400 tracking-[3px] uppercase italic">PROTOCOL 2026</Typography>
                <Typography weight="black" className="text-xl text-white italic tracking-tighter uppercase">TERMS OF USE</Typography>
              </View>
           </View>
        ...
        <View className="bg-white/5 p-8 rounded-[40px] border border-white/10 mb-8">
           <Typography weight="bold" className="text-xs text-white/40 italic leading-6 uppercase tracking-wider text-center">
             USAGE OF <Typography weight="black" className="text-white">EFOUR ELURU</Typography> (EFOUR-ELURU.COM) IS SUBJECT TO THESE TERMS & CONDITIONS. ACCEPTANCE IS IMPLIED BY USING THE SITE.
           </Typography>
        </View>
        ...
        <View className="flex-row flex-wrap justify-between">
          ...
          <View className="w-[48%] bg-white/5 p-5 rounded-[32px] border border-white/5 mb-4">
            <View className="flex-row items-center space-x-3 mb-4">
              ...
              <Typography weight="black" className="text-[10px] text-white italic tracking-widest uppercase flex-1">CONTACT INFORMATION</Typography>
            </View>
            <Typography weight="black" className="text-[7px] text-white/30 tracking-widest italic lowercase mb-2">CEO@EFOUR-ELURU.COM</Typography>
            <Typography weight="black" className="text-[7px] text-white/30 tracking-widest uppercase italic">+91 70369 23456</Typography>
          </View>
        </View>
      ...
      <View className="absolute bottom-10 inset-x-6">
         <Pressable onPress={() => router.back()} className="bg-indigo-600 rounded-[32px] py-6 flex-row items-center justify-center shadow-2xl shadow-indigo-600/40 active:bg-indigo-700">
            <Typography weight="black" className="text-[11px] text-white tracking-[5px] uppercase">I AGREE</Typography>
